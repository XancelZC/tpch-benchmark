...
> **本报告定位**：本文件是项目整体报告，说明方案设计、实现、测试过程和结论。
> **benchmark 运行报告**（测试环境、总体对比、逐查询比较、Top 慢 SQL、失败记录、限制）位于：
> `results/final/20260804T040518Z/report.md`，由每次运行自动生成，数据与 `raw_records.csv` 同源。

## 1. 方案设计

### 1.1 核心目标

实现一个配置驱动的多数据库查询测试工具，用于 FDE 场景中的客户 PoC、迁移评估、竞品分析和性能问题定位。
...
（MVP 版本中曾发现 MySQL Q8 因缺少逻辑主键访问路径产生灾难性执行计划，约 120 秒超时。补齐等价索引后，Q8 恢复到百毫秒级，并重新完成四库预检）
...
```text
results/final/20260804T040518Z/
├── raw_records.csv
...
|---|---:|---:|---:|---:|---:|
| YMatrix vs Greenplum | 2.31x | 2.32x | 2.47x | 3.65x | 18/4/0 |
| YMatrix vs PostgreSQL | 0.70x | 0.95x | 0.78x | 1.03x | 11/11/0 |
| YMatrix vs MySQL | 2.28x | 2.87x | 2.00x | 3.51x | 15/7/0 |

### 4.1 结论边界

- YMatrix 在本次小规模、单节点、统一 amd64、indexed profile 下，对 Greenplum 的整套串行 workload 总耗时约为 2.31 倍优势；逐查询中位数和几何均值方向一致；
- YMatrix 与 PostgreSQL 总耗时比为 0.70x，逐查询胜负为 11/11，考虑到 Rosetta 转译损耗约 20-30%因素，实际 YMatrix 性能会更优；
- YMatrix 对 MySQL 的总耗时比为 2.28x，几何均值为 2.00x，有 7 条查询 MySQL 更快；
...
MVP 版本自定义随机生成器破坏了 partsupp 和 lineitem 的关系，也导致部分查询谓词天然为空。该问题已经通过固定 dbgen 数据链路解决，旧结果全部废弃。
...
7. 可选：为四库按各自最佳实践调优参数，对比"调优后"与"默认"两档。
